...
hyper_fn="param-botsort-results.csv"
...
def create_tracker_file(exp_id=None,\
    ...
    with open(tracker_config_file, 'w') as f:
        config = '# Ultralytics YOLO 🚀, AGPL-3.0 license\n' + \
                ...
                '\n'+\
                '# BoT-SORT settings\n'+\
                f'gmc_method: {gmc_method}  # method of global motion compensation\n'+\
                '# ReID model related thresh (not supported yet)\n'+\
                f'proximity_thresh: {proximity_thresh}\n'+\
                f'appearance_thresh: {appearance_thresh}\n'+\
                f'with_reid: {with_reid}'
        f.write(config)
...
def create_folders(exp_id,max_frames):
    ...
    if not os.path.exists(hyper_fn):
        with open(hyper_fn,'w') as f:
            f.write('exp_id,max_frames,weights_fn,tracker_type,track_high_thresh,track_low_thresh,new_track_thresh,track_buffer,match_thresh,gmc_method,proximity_thresh,appearance_thresh,with_reid,')
...
def read_hota(exp_id=None,\
    ...
    with open(fn) as f:
        ...
        with open(hyper_fn,'a') as g:
            g.write(f"{exp_id},{max_frames},{weights_fn},{tracker_type},{track_high_thresh},"+\
                    f"{track_low_thresh},{new_track_thresh},{track_buffer},{match_thresh},"+\
                    f"{gmc_method},{proximity_thresh},{appearance_thresh},{with_reid},")
            g.write(line)
            g.write('\n')



df=pd.read_csv('param-botsort.csv')
...
for index, row in df.iterrows():
    ...
    create_tracker_file(exp_id=row.exp_id,\
                        ...
                        gmc_method=row.gmc_method, \
                        proximity_thresh=row.proximity_thresh,\
                        appearance_thresh=row.appearance_thresh,\
                        with_reid=row.with_reid)
